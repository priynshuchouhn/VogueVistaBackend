...
let statusUpdateTimeOut: string | number | NodeJS.Timeout | undefined;

export const getOrders = async (req: IRequest, res: Response, next: NextFunction) => {
    const userId = req.user.userId
    try {
        const lstOrder = await Order.find({ userId: userId }).populate(['products', 'shippingAddress']);
...
export const addOrder = async (req: IRequest, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        const newOrder = await tempOrder.populate(['products', 'shippingAddress'])
